...
//DESCRIPCIÓN: Vista heredada (FrmBase) para la entidad Categoría; implementa IPanelCRUD
...
namespace Actividad_2._1_Diseño_e_implenacion_de_Modelos
{
    public partial class FrmCategoria : FrmBase, IPanelCRUD
    {
        ...
        private readonly Categoria _modelo = new Categoria();
        ...
        public void EjecutarGuardar()
        {
            try
            {
                Categoria nuevo = LeerFormulario();
        ...
        public void EjecutarBuscar(string id, ErrorProvider alerta)
        {
            ...
            if (_modelo.ConsultarRegistro(id) is Categoria encontrado)
            {
            ...
            {
                alerta.SetError(txtId, $"No existe ningún registro de Categoría con el ID '{id}'.");
        ...
        public void EjecutarActualizar()
        {
            try
            {
                Categoria actualizado = LeerFormulario();
        ...
        public void EjecutarEliminar(StatusStrip barraEstado)
        {
            ...
            {
                MostrarEstado(barraEstado, $"No existe ningún registro de Categoría con el ID '{id}'.");
                return;
            }

            if (!Confirmar($"¿Eliminar el registro de Categoría '{id}'?"))
            {
            ...
            MostrarEstado(barraEstado, $"Registro de Categoría '{id}' eliminado correctamente.");
        }
        ...
        private void MapearAFormulario(Categoria obj)
        {
            txtId.Text = obj.IdCategoria.ToString();
            txtNombreCategoria.Text = obj.NombreCategoria;
        ...
        private void MostrarResultado(string encabezado, Categoria obj)
        {
            txtResultado.Text = $"{encabezado}\r\n{obj}\r\n" +
                $"-> Etiqueta Normal: {obj.ObtenerEtiqueta()}\r\n" +
                $"-> Etiqueta Especial: {obj.ObtenerEtiqueta("ODS-2026")}";
        }
